...
def execute_feature_selection(runs, methods, n_features,n_seed,splits):
    for meth in methods:
        method = [meth] 
        result = Parallel(n_jobs=-1)(delayed(execute_feature_selection_a_run)(run, method) for  run in runs)
        produce_features(result, n_features, method, n_seed,splits)
        with open(meth+".txt","wb") as  fp:
