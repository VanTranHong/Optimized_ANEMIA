...
def bag_naive_bayes(X_train,X_test,y_train,y_test):
    ...
    estimators = [100,200,300,400,500,800,1000]

    df = pd.DataFrame(columns=['Estimators','Confusion Matrix'])
    rows = []
    ...
    for n_est in estimators:
        ...
        tn, fp, fn, tp = confusion_matrix(y_test, predicted_labels, labels=[0,1]).ravel()
        convert_matrix = [tn,fp,fn,tp]
        rows.append([n_est,  convert_matrix])
    for i in range(len(rows)):
        df = df.append({'Estimators':rows[i][0],'Confusion Matrix':rows[i][1]}, ignore_index=True)
    return df
